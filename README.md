...
These are my dotfiles. They contain my configurations of Neovim, tmux, ZSH and more. On my Mac, there are no developer tools installed except Vagrant (with the [landrush](https://github.com/phinze/landrush) plugin), Packer, iTerm 2 and Xcode. My dotfiles contains a Vagrantfile to provision the Vagrant box running in VMWare Fusion. The box is build on Atlas by Packer (You can find the code for that [here](https://github.com/yakshed/base-vm)). So when I want to start developing on my machine, I do a `vagrant up && vagrant ssh` and then I work inside the Vagrant box. From time to time I run a `vagrant destroy` to keep everything clean.
